...
<link rel="stylesheet" href="css/sloution.css"/>
...
<!--start内容-->
  <section>
       ...
       <!--start解决方案内容模块--> 
        <?php
				$dosql->Execute("SELECT * FROM `#@__infoimg` WHERE classid=22 AND  delstate='' AND checkinfo=true ORDER BY orderid DESC LIMIT 0,3");
				while($row = $dosql->GetArray())
				{
					if($row['linkurl'] != '')$gourl = $row['linkurl'];
					else $gourl = 'javascript:;';
